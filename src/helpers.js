...
hbs.registerHelper("showCourses", ()=>{
	try{
		list_cursos = require("./cursos.json");
	}catch(error){
		list_cursos = [];
	}

	list_cursos=list_cursos.filter(course => course.availability == "Disponible");

	var result = "<div>";

	list_cursos.forEach(course => {
...
hbs.registerHelper("showEnrolled", ()=>{
	...
	try{
		list_enrolled = require("./inscritos.json");
	}catch(error){
		list_enrolled = [];
	}
	...
	list_cursos.forEach(course => {
		...
		people.forEach(student =>{
			result = result + "<tr><td>" + student.doc +"</td><td>" + student.name +"</td><td>" + student.email + "</td><td>" + student.phone + "</td><td><a class='btn btn-danger' href='../enrolled/delete?student=" + student.doc + "&course=" + course.id + "'>Eliminar</a></td></tr>";
		});
